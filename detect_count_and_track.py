...
def detect(save_img=False):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            #insert mysql(counting - tmp,time.time())

            # 获取今天的时间戳
            today_timestamp = int(time.time()) % 86400
            if (today_timestamp - int(t0)) % 60 == 0:
                # 要插入的数据
                data = [
                    (counting - tmp, today_timestamp),  # 第一条数据 (counting, time)
                ]

                # 插入数据到数据库
                cursor.executemany("INSERT INTO your_table_name (counting, time) VALUES (?, ?)", data)
                cursor1.executemany("INSERT INTO car_data (car_count ,timestamp) VALUES (%s, %s)", data)
                # 提交事务并关闭连接
                conn.commit()
                connect.commit()
                #0 (tmp)1(   )2
